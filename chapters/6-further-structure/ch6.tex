...
[WORK: topologically ordered system with boundary. DON'T use the categorical ``$\cC$". Use the words ``topologically ordered system". Keep with this convention for the whole section. I'm not assuming that the reader knows any category theory.]
...
This fusion can be nondeterministic. That is, repeating the fusion of two boundary defects can yield different results. In this sense boundaries can host non-abelian defects. Importantly, we will find that abelian bulk theories can host non-abelian boundary defects! This is the heart of our proposal for quantum computing with $\fD(\bZ_2)$ topological order. Instead of working with the abelian anyons, we work with the nonabelian boundary defects!
...
\item Characterization of Lagrangian algebras in $\cZ(\Vec_{G})$;
\end{itemize}
]
...
\subsubsection{Module and bimodule categories}
...
\text{by the modular category $\cB$}\\
...
\text{$\cB\cong \cZ(\cC)$ for some}\\
\text{spherical fusion category $\cC$}
}
\right).
\end{equation*}

For this reason, we will restrict our attention to \textit{doubled} topological order for the rest of the section. That is, modular categories $\cB$ which are equivalent to $\cZ(\cC)$ for some spherical fusion category $\cC$. We will simplify notion for doubled topological order. We will write

[WORK: bulk theory with letter $\cC$ in it $=$ topological ordered described by $\cZ(\cC)$.]

This now leads us to the big question: what, algebraically, are boundaries theories in the $\cZ(\cC)$ topological order?

[WORK: The answer is $\cC$-module categories. I am suspecting that to motivate this answer one needs to take some physical principles from the string-nets developped in the Levin-Wen model. I haven't written the Levin-Wen section yet, so this can wait until that.]

More generally, a domain wall between the phases described by $\cZ(\cC)$ and $\cZ(\cD)$ will be a $(\cC,\cD)$-bimodle category. [WORK: motivate this as well].

We rigorously define the above intuitive notions as follows.

\begin{defn}[Left module category] A \textit{left module category} over a fusion category $\cC$ is the following data:

\begin{enumerate}
\item A $\bC$-linear category $\cM$;
\item (Action) A functor $\otimes: \cC \times \cM \xrightarrow{} \cM$;
\item (Left associator) A natural isomorphism $m: \left((\--)\otimes (\--)\right)\otimes (\--) \xrightarrow{\sim} (\--)\otimes \left((\--)\otimes (\--)\right)$ of functors $\cC\times \cC\times \cM \xrightarrow{} \cM$;
\item (Unit) A natural isomorphism $\lambda_m:\bone \otimes (\--)\xrightarrow{} (\--)$ of functors $\cM\xrightarrow{}\cM$.
\end{enumerate}
...
\item There is an equivalence $\cC \cong \Vec_{\bC}^n$ as $\bC$-linear categories for some integer $n\geq 1$.

\item The functor $\otimes$ is $\bC$-linear.

\item For all $A,B,C\in \cC$ and $M\in \cM$ the diagram
...
\item For all $A\in \cC$ and $M\in \cM$, the diagram
...
\[\begin{tikzcd}
	{(A\otimes \bone)\otimes M} & {A\otimes (\bone \otimes M)} \\
	{A\otimes M}
	\arrow["{m_{A,\bone,M}}", from=1-1, to=1-2]
...
\begin{defn}[Bimodule category] A \textit{$(\cC,\cD)$-bimodule category} $\cM$ over fusion categories $\cC,\cD$ is the following data:

\begin{enumerate}
\item The structure of a right $\cC$-module on $\cM$;
\item The structure of a left $\cD$-module on $\cM$;
\item (Middle associator) A natural isomorphism $b: ((\--)\otimes (\--))\otimes (\--)\xrightarrow{\sim} (\--)\otimes ((\--)\otimes (\--))$ of functors $\cC\times\cM \times \cD \xrightarrow{} \cM$;
\end{enumerate}

Such that:

\begin{enumerate}
\item For all $A,B\in \cC$, $M\in \prescript{}{\cC}\cM_{\cD}$, $A',B\in \cD$, the diagrams
...
Given two fusion categories $\cC,\cD$ and a $(\cC,\cD)$-bimodule categories $\cM$, we define $\cM$ to be a \textit{simple} module if for all simple objects $N,M\in \cM$ there exists some $A\in \cC$ such that $\Hom(N,A\otimes M)\neq 0$. This condition of irriducibility corresponds exactly the condition of stability of boundaries. Hence, we arrive at the following correspondance:
...
\begin{defn}[Left module functor] A \textit{left module functor} between left $\cC$-module categories $\cM$, $\cN$ is the following data:

\begin{enumerate}
\item A functor $F: \cM \xrightarrow{}  \cN$;
\item A natural isomorphism $s:F((\--)\otimes (\--))\xrightarrow{\sim}(\--)\otimes (\--)$ of functors $\cC\times \cM\xrightarrow{} \cN$.
\end{enumerate}
...
\item For all $A,B\in \cC$, $M\in \cM$, the diagram
...
\[\begin{tikzcd}
	{F(\bone\otimes M)} & {\bone\otimes F(M)} \\
	& {F(M)}
	\arrow["{s_{\bone,M}}", from=1-1, to=1-2]
	\arrow["{F(\lambda^\cM_{m,M})}"', from=1-1, to=2-2]
	\arrow["{\lambda^{\cN}_{m,F(M)}}", from=1-2, to=2-2]
\end{tikzcd}\]
...
A right module functor is defined in complete analogy to left module functor. A $(\cC,\cD)$-bimodule functor is defined to be a functor which is simulaneously equipped with the structure of a left and right module functor. We denote by $\Fun_{\cC}(\cM, \cN)$, the space of $\cC$-modue functors from $\cM$ to $\cN$, and we denote by $\Fun_{\cC | \cD}( \cM, \cN)$ the space bimodule functors.
...
\begin{defn}[Natural transformation of left module functors] A \textit{natural transformation of left module functions} between left $\cC$-module functors $F,G:\cM\to \cN$ is a natural transformation $\eta:F\to G$ such that for all $A\in \cC$, $M\in \cM$
...
The fusion of boundary defects is clear. Given two boundary defects $F: \cM\to \cM'$ and $G: \cM'\to \cM''$ the composition $(G\circ F): \cM\to \cM''$ gives another boundary defect. This is the correct physical choice becase [WORK: depends on the physical motivation I gave for defects corresponding to functors].

The fusion of domain walls is a bit more sublte. It requires the notion of relative tensor product:

\begin{defn}[Relative Deligne tensor product] [WORK: give definition]
\end{defn}

Physically, we can define fusion of domain walls $\cM$ and $\cN$ to be the relative Deligne tensor product $\cM\boxtimes_{\cK}\cN$, where $\cM$ is a $(\cC\,\cK)$-bimodule and $\cN$ is a $(\cK,\cD)$-bimodule. This is the correct definition because [WORK: give physical motivation].
...
[WORK: add folding trick. The flat plane should have the label $(\cC,\cD)$-bimodules and the folded plane shuld have the label $(\cC\boxtimes \overline{\cD})$-modules.]

This is codifed mathematically in the following proposition:

\begin{prop}[Folding trick] Let $\cC,\cD$ be fusion categories. Given a $(\cC,\cD)$-bimodule $\cM$, define a left ($\cC\boxtimes \overline{\cD}$)-module structure on $\cM$ via the formula

$$(A\boxtimes \overline{B})\otimes M = (A\otimes M)\otimes B$$

for any $A\in \cC$, $B\in \cD$, $M\in \cM$. This is a well-defined left $(\cC\boxtimes \overline{\cD})$-module structure. This assignment induces a bijection between the set of  $(\cC,\cD)$-bimodules and the set of left $(\cC\boxtimes \overline{\cD})$-modules.
\end{prop}
\begin{proof} This is a straightforward and unenlightening exercise.
\end{proof}


Next, we take a closer look at simple modules. That is, module categories with no proper nontrivial submodule categories. Given two categories $\cM$ and $\cN$, we can define their Cartesian product $\cM\times \cN$, whose objects are pairs $(A,B)$, $A\in \cM$, $B\in \cN$. To avoid confusion with the Deligne tensor product, we will use the notation $\boxplus$ to denote the Cartesian product in this context, and refer to it as the direct sum. This is consistent with our genereal principles of higher linear algebra (see section [ref]). If $\cM$ and $\cN$ are left $\cC$-module categories, then $\cM\boxplus \cN$ can be equipped with the structure of a left $\cC$-module via the action
...
\begin{lem} Let $\cC$ be a fusion category. Let $\cM$ be a left $\cC$-module. For all $A\in \cC$ and $N,M\in \cM$, we have that

$$\Hom_{\cM}(N,A\otimes M)\cong \Hom_{\cM}(A^*\otimes N, M)$$
...
\begin{prop} Let $\cC$ be a fusion category. Every left $\cC$-module category is equivalent to a direct sum of simple $\cC$-module categories.
\end{prop}
\begin{rem} The usual way of summarizing this proposition is by saying that the space of left $\cC$-modules is \textit{semsimple}.
\end{rem}
\begin{proof} Let $\cM$ be a left $\cC$-module. Denote by $\cL_{\cM}$ the set of isomorphism classes of simple objects in $\cM$. We define a equivalence relation $\sim$ on $\cL_{\cM}$ by saying that $[N]\sim [M]$ if there exists some $A\in \cC$ such that $\Hom(N,A\otimes M)\neq 0$. This relation is reflexive since $\Hom(M,\bone\otimes M)\neq 0$, it is transitive since if $\Hom(M,A\otimes M')\neq 0$ and $\Hom(M',B\otimes M'')\neq 0$, then $\Hom(M,A\otimes B\otimes M'')\neq 0$ by composing morphisms. Seeing as $\cM\cong \Vec_{\bC}^n$ for some $n\geq 1$ as a category, we have that $\Hom(N,M)\cong \Hom(M,N)$ as vector spaces for all $N,M\in \cM$. Hence, the equivalence relation is symmetric by lemma [ref].

For all $i\in \cL_{M}/\sim$, we define a full subcategory of $\cM$ via

$$\cM_{i}=\left\{\left.M\in \cM\right| M\cong \bigoplus_{[N]\in i}n_{N}\cdot N,\text{ for some integers } n_{A}\geq 0\right\}.$$

Now, our equivalence relation gaurantees that $\cM_{i}$ is closed under the action of $\cC$ for all $i\in \cL_{\cM}/\sim$. That is, it forms a left $\cC$-submodule of $\cM$. We claim that the map

\begin{align*}
\bplus_{i\in \cL_{\cM}/\sim}\cM_{i}&\xrightarrow{}\cM\\
(M_{i})_{i\in \cL_{\cM}/\sim}&\mapsto \bigoplus_{i\in \cL_{\cM}/\sim}M_{i}
\end{align*}

is an equivalence of categories. It is faithful by the general properties of the direct sum. It is full because $\Hom(M_i,M_j)=0$ for all $i\neq j$ since $M_i$, $M_j$ are the direct sums of disjoint sets of simple objects, and hence every morphisms can be broken into its $i$-components for each $i\in \cL_{\cM}/\sim$. It is essentially surjective because every simple objects appears in the category $\cM_{i}$ for some $i\in \cL_{\cM}/\sim$. Hence, it is an equivalence of categories by proposition [ref] so our proof is complete.
\end{proof}

With propositions [ref] and [ref] in hand, we can set up more directly the theory of fusion of domain walls. Let $\cC$, $\cK$, $\cD$ be fusion categories. Let $\cM$ be a $(\cC,\cK)$-bimodule and let $\cN$ be a $(\cK,\cD)$ bimodule. We have an equivalence

$$\cN\boxtimes\cM \cong \bplus_{[\cM']} n^{\cN,\cM}_{\cM'}\cdot \cM',$$

where $[\cM']$ runs over the set of equivalence classes of simple $(\cC,\cD)$-bimodules. This is completely analogus to the algebraic theory of fusion of anyons introduced in section [ref]. One of the major differenes is that we are now up a categorical level. Instead of fusing objects in a category, we are fusing the categories themselves.
...
\begin{prop} Let $\cC$ be a fusion category. Let $\cM$ be a left $\cC$-module. Define

$$\cC^{\vee}_{\cM}=\Fun_{\cC}(\cM,\cM).$$

We call $\cC^{\vee}_{\cM}$ the \textit{dual of $\cC$ with respect to $\cM$}. We define a monoidal structure on $\cC^{\vee}_{\cM}$ via the formula

$$F\otimes G = G\circ F.$$

Along with a natural choice of rigid structure, $\cC^{\vee}_{\cM}$ has the structure of a fusion category.

If $\cC$ is a spherical fusion category, then $\cC^{\vee}_{\cM}$ is naturally equipped with the structure of a spherical fusion category.
\end{prop}
...
I'm in trouble the proof uses Yetter cohomology and other scary-looking tools. Is there a simpler proof? The hard part is showing that if $\cN,\cM$ are module categories the space of module functors is equivalent to $\Vec_{\bC}^n$ for some $n\geq 0$. In particular, we need to show that there are finitely many isomorphism classes of simple module functors. How do I do it?! It's a sort of generalized Ocneanu rigidity.
]
\end{proof}

When the source and target are different, we have the follwing result:

\begin{prop} Let $\cC$ be a fusion category. Let $\cN$, $\cM$ be left $\cC$-modules. The category $\Fun_{\cC}(\cN,\cM)$ is naturally equipped with the structure of a left $\cC_{\cN}^{\vee}$-module and a right $\cC_{\cM}^{\vee}$-module.
\end{prop}
...
\begin{prop} Let $\cC$ be a fusion category. There are finitely many equivalence classes of simple left $\cC$-modules.
\end{prop}
...
\begin{thrm} Let $\cC$ be a fusion category. Let $\cM$ be a left $\cC$ module. The map

\begin{align*}
\can: \cC &\xrightarrow{\sim } \left(\cC^{\vee}_{\cM}\right)^{\vee}_{\cM}\\
A &\mapsto \left(M\mapsto A\otimes M\right)
\end{align*}

is an equivalence of categories. The $\cC^{\vee}_{\cM}$-module functor structure on $\can_{A}$ is defined for all $A\in \cC$, $M\in \cM$, $F\in \cC^{\vee}_{\cM}$ by

$$s^{\can_{A}}_{F,M}:\can_{A}(F\otimes M )= A\otimes F(M)\xrightarrow{s^{-1}_{A,M}}F(A\otimes M)=F\otimes \can_{A}(M).$$

If $\cC$ is a spherical fusion category, $\can$ is an equivalence of spherical fusion categories.
\end{thrm}
\begin{rem} This result is an analogue of the \textit{double centralizer theorem} from classical representation theory.
\end{rem}
...
\begin{cor} Let $\cC$ be a fusion category and let $\cM$ be a left $\cC$-module. The module $\cM$ is simple as a right $\cC_{\cM}^{\vee}$-module.
\end{cor}
\begin{proof}.[WORK: do proof]
\end{proof}

Of course, the above discussion gaurantees a proper theory of fusion of boundary defects. Suppose that $\cC,\cD$ are fusion categories, and $\cM$, $\cM'$, $\cM''$ are simple $(\cC,\cD)$-bimodules. Suppose that $F:\cM\to \cM'$ and $G:\cM'\to\cM''$ are simple $(\cC,\cD)$ bimodule functors. Since $\Fun_{\cC|\cD}(\cM,\cM'')$ is equivalent to $\Vec_{\bC}^n$ for some $\geq 0$ as a $\bC$-linear category, we have a decomposition

$$G\circ F \cong \bigoplus_{[H]}n^{F,G}_{H}\cdot H$$

in $\Fun_{\cC,\cD}(\cM,\cM'')$ where $[H]$ runs over isomorphism classes of simple functors in $\Fun_{\cC|\cD}(\cM,\cM'')$. Again, this is completely analagous to the fusion of anyons.
...
We already know one thing about doubled topological order - its anyons are described by the Drinfeld center $\cZ(\cC)$ of the input spherical fusion category $\cC$. These anyonic bulk excitations can interact with the boundaries, and produce non-trivial effects. This means that the category $\cZ(\cC)$ neccecarily has deep connections to the theory of boundaries, and thus mathematically will have a module-theoretic interpretation. This will also help answer the question of why the Drinfeld center $\cZ(\cC)$ is the category which describes anyons in the Levin-Wen model.
...
However, we know from our first discussion of the Levin-Wen model that anyon types should correspond to simple objects in $\cZ(\cC)$. Hence, we conclude that simple objects in $\cZ(\cC)$ should correspond to simple objects in $\Fun_{\cC|\cC}(\cC,\cC)$. Of course, this equivalence should repsect fusion, duality, braiding, and all the other algebraic structures of anyons. Thus, if our boundary theory is correct, it predicts an equivalence of categories $\cZ(\cC)\cong\Fun_{\cC|\cC}(\cC,\cC)$. This mathematical prediction is accurate, by the following proposition:

\begin{prop} Let $\cC$ be a fusion category. The map

\begin{align*}
\Fun_{\cC|\cC}(\cC,\cC)&\xrightarrow{\sim}\cZ(\cC)\\
...
\[\begin{tikzcd}
	{\upbeta_{F(1),\--}:F(\bone)\otimes B} & {F(\bone\otimes B)=F(B)=F(B\otimes \bone)} & {B\otimes F(\bone)}
	\arrow["{s^R_{\bone,B}}", from=1-1, to=1-2]
	\arrow["{(s^L_{B,\bone})^{-1}}", from=1-2, to=1-3]
\end{tikzcd}\]

and $s^L$, $s^R$ are the compatibility maps implicit in the left/right module functor structure of $F$.

If $\cC$ is a spherical category, then the map $\Fun_{\cC|\cC}(\cC,\cC)\xrightarrow{\sim}\cZ(\cC)$ induces an equivalence as pre-modular categories.
\end{prop}
\begin{proof}. [WORK: do proof]
\end{proof}

This equivalence of categories gives the proper interpretation for $\cZ(\cC)$ in the theory of boundaries. If we wanted we could have taken it as a \textit{defn} of $\cZ(\cC)$. We now examine how anyons interact with boundaries to get more module-theoretic properties of the Drinfeld center.

Suppose $\cC$, $\cD$ are spherical fusion categories, and $\prescript{}{\cC}\cM_{\cD}$ is a bimodule. If a $A\in \cC$ is a simple object describing an anyon, we can imagine trying to push $A$ from $\cC$ to $\cD$ through the boundary theory $\prescript{}{\cC}\cM_{\cD}$ :

[WORK: add picture: first its on one side, then a bubble starts to form, then the bubble detaches. It leaves behind a dotted line with label $\prescript{}{\cD}\cM^{op}_{\cC}\boxtimes_{\cC}\prescript{}{\cC}\cM_{\cD}$ ]

Suppose that the domain wall is \textit{invertible}. This means that $\prescript{}{\cD}\cM^{op}_{\cC}\boxtimes_{\cC}\prescript{}{\cC}\cM_{\cD}\cong \cD$. Thus, the anyon can pass directly through the boundary, and the boundary can re-close without leaving a trace of the fact that the anyon went through.

This gives an assignment of anyons in the $\cZ(\cC)$ to anyons in the $\cZ(\cD)$ phase. This assignement is reversible, because anyons in the $\cZ(\cD)$ phase can pass through the domain wall in the other direction. Thus, our boundary theory predicts that every invertible bimodule $\prescript{}{\cC}\cM_{\cD}$ will induce an equivalence of categories $F_{\cM}:\cZ(\cC)\xrightarrow{\sim}\cZ(\cD)$. We verify this prediction with a theorem. Before we can prove it, we need a lemma.

\begin{lem} Let $\cC$ be a fusion category. The forgetful functor

\begin{align*}
\cZ(\cC)&\xrightarrow{} \cC\\
(A,\upbeta_{A,\--})&\mapsto A
\end{align*}

is surjective onto the objects of $\cC$.
\end{lem}
\begin{proof}.[WORK: this proof is steeped in the theory of module categories.]
\end{proof}

\begin{thrm} Let $\cC$, $\cD$ be spherical fusion categories. There is a bijection

$$
\left(
\substack{
\text{invertible $(\cC,\cD)$-bimodule}\\
\text{categories }\cM}
\right)
\xleftrightarrow{\sim}
\left(
...
\text{categories }\cZ(\cC)\xrightarrow{\sim}\cZ(\cD)}
\right),$$

defined by sending a $(\cC,\cD)$-bimodule category $\cM$ to the functor $F_{\cM}:\cZ(\cC)\to\cZ(\cD)$ defined by

\begin{align*}
F_{\cM}: \cZ(\cC)\cong &\Fun_{\cC|\cC}(\cC,\cC) \xrightarrow{} \Fun_{\cD|\cD}(\cD,\cD)\cong \cZ(\cD).\\
G&\mapsto \left(\cD\xrightarrow{}\cM^{op}\boxtimes_{\cC}\cC\boxtimes_{\cC}\cM \xrightarrow{\id\boxtimes G \boxtimes \id}\cM^{op}\boxtimes_{\cC}\cC\boxtimes_{\cC}\cM \xrightarrow{}\cD\right)
\end{align*}

For all triples $\cC$,$\cK$, $\cD$ of fusion categories, $(\cC,\cK)$-bimodules $\cM$, and $(\cK,\cD)$-bimodules $\cN$, we have that

$$F_{\cM\boxtimes_{\cK}\cN}=F_{\cN}\circ F_{\cM}.$$

If $\cC$ and $\cD$ are spherical fusion categories, then the assignement $\cM\mapsto F_{\cM}$ induces a bijection.

[WORK: what is the correct statement? Do I have to reduce the class of bimodules I am considering?]

\end{thrm}
\begin{rem} This theorem is originally due to Etingof-Nikshych-Ostrik \cite{etingof2010fusion}. It was observed in the theory of gapped boundaries by Kitaev and Kong \cite{kitaev2012models}. The proof presented here is original.
\end{rem}
\begin{proof}
.[WORK: do proof. The first direct is making sure this is a well-defined map and filling in details. That's not very hard.

The big question is showing that this is a \textit{bijection}. That means that we need an inverse to this map. The inverse is suprisingly cute and nice. The forgetful functor $\cZ(\cC)\to \cC$ induces a $\cZ(\cC)$-bimodule structure on $\cC$. Given any equivalence $F:\cZ(\cC)\xrightarrow{\sim}\cZ(\cD)$, we can thus construct the tensor product

$$\cC \boxtimes_{\cZ(\cC)\cong \cZ(\cD)}\cD.$$

This tensor product is a $(\cC,\cD)$ bimodule. The fact that it is an invertible bimodule uses in a key way the fact that the forgetful functors $\cZ(\cC)\to \cC$ and $\cZ(\cD)\to \cD$ are surjective. It's not hard to show that these two constructions are inverses to one another. This gives the proof.
]
\end{proof}


As a corollary we find the following theorem, which was first proved by Kitaev and Muger and communicated by Etingof-Nikshych-Ostrik \cite{etingof2011weakly}. We recall that two fusion categories $\cC$, $\cD$ are called \textit{Morita equivalent} if their Drinfeld centers are equivalent as braided fusion categories

[WORK: What is the correct statement of this theorem for spherical fusion categories?]

\begin{cor} Two fusion categories $\cC$, $\cD$ are Morita equivalent if and only if there exists an invertible bimodule category between them.
\end{cor}
...
[WORK: Out of all this we get Lagrange's theorem for fusion categories, that given a fusion $\cC$ with full subcategory $\cC$, the ratio $\FPdim(\cD)/\FPdim(\cC)$ is an algebraic integer. Do I care? Should I include this? Maybe I should have a section with a proof of Muger's theorem plus a proof of Lagrange's theorem for fusion categories.]
...
[WORK: I would like to include the Curie principle - the symmetry of the causes are to be found in the effects! The symmetries $\Aut^{\otimes}(\cC)$ are nessecarily interesting objects. Why? Curie principle! The curie principle in this case manifests in the possible defects which can appear. While $\cC$ and $\cC\otimes \overline{\cC}$ might \textit{look} like very similar categories, $\cC\otimes \overline{\cC}$ may have way more symmetries than $\cC$ and hence might look like a better candidate for TQC in this guise
...
$$\cC <-- confinement, defectification --> \cC^{\times}_{G} $$
...
\begin{defn}[Categorical group action] A \textit{categorical group action} of a finite group $G$ on a fusion category $\cC$ is the following data:

\begin{enumerate}
\item A function $\rho: G \xrightarrow{}\Aut^{\otimes}(\cC)$;
...
\item There is an equality of functors $ \rho(e)=\id_{\cC}$, where $e\in G$ is the identity element\footnote{The condition is added for simplicity of notation down the line - the axioms already imply that there is an equivalence $\rho(e)\cong \id_{\cC}$.}.
...
\left(
\substack{
\text{triples $(\cC,G,\rho)$,}\\
\text{where $\cC$ is a modular category, $G$ is a finite group}\\
\text{and $\rho$ is a braided categorical group action}
}
\right).
\end{equation*}

Algebraically, the physical phenominon of \textit{symmetry breaking} is extremly clear. It is the act of taking a symmetry enriched phase $(\cC,G,\rho)$ and passing some subgroup $H\leq G$,  yield $(\cC,H,\left.\rho\right|_{H})$. Passing all the way to the trivial group yields the ordinary topological phase $\cC$.
...
\begin{defn}[$G$-graded fusion category] A \textit{$G$-graded fusion category} over a finite group $G$ is the following data:

\begin{enumerate}
\item A fusion category $\cC$;
\item A full subcategory $\cC_{g}$ of $\cC$ for each $g\in G$.
\end{enumerate}
...
\begin{align*}
\bplus_{g\in G}\cC_g &\xrightarrow{\sim} \cC\\
(A_g)_{g\in G}&\mapsto \bigoplus_{g\in G}A_g
\end{align*}

is an equivalence of categories;

\item The tensor product on $\cC$ restricts to a map $\otimes: \cC_g\times \cC_h \xrightarrow{}\cC_{gh}$;

\item (Faithfullness\footnote{This condition is not included in the definition by many authors.}) The subcategories $\cC_g$ are nonzero for all $g\in G$.
\end{enumerate}

\raggedleft\qedsymbol{}
\end{defn}

\begin{defn}[$G$-crossed braided fusion category] A \textit{$G$-crossed braided fusion category} over a finite group $G$ is the following data:

\begin{enumerate}
\item A $G$-graded fusion category $\cC^{\times}_{G}$ with $g$-component $\cC_{g}$;
\item A categorical group action $\rho$ of $G$ on $\cC^{\times}_{G}$;
\item A natural isomorphism

$$\beta_{g,A,B}:A\otimes B \xrightarrow{}\rho(g)(B)\otimes A$$

for all $A\in \cC_{g}$, $B\in \cC^{\times}_{G}$, $g\in G$.
\end{enumerate}
...
$$\rho(g)(\cC_h)\subseteq \cC_{g h g^{-1}}.$$
]
...
\begin{defn}[$G$-crossed modular category] A \textit{$G$-crossed modular category} over a finite group $G$ is the following data:

\begin{enumerate}
\item A $G$-crossed braided fusion category $\cC_{G}^{\times}$;
\item A spherical structure on $\cC_{G}^{\times}$;
\end{enumerate}
...
\item $\cC_{e}$ is a modular category, where $e\in G$ is the identity element. [WORK: say this nicer. Should I introduce better notation for group elements?]
\end{enumerate}

\raggedleft\qedsymbol{}
\end{defn}

We observe from our definitoins that the identity graded component $\cC_{e}$ of any $G$-crossed modular category is itself a modular category. The assignment $\cC^{\times}_{G}\mapsto \cC=\cC_{e}$ corresponds to the physical transition from a phase where defects exist and can be freely manipulated, and a phase in which no free defects can be made. This is known physically as \textit{confinement}. This gives us the following picture:
...
$$ \cC^{\times}_{G}  <-- condensation, gauging --> \left(\cC^{\times}_{G}\right)^{G}$$
...
\begin{prop} Let $\cC$ be a fusion category with a categorical $G$ action. Define a category $\cC^{G}$ as follows. It's objects are pairs $(A,\gamma)$, where $A\in \cC$ is an object and

$$\gamma_{g,A}:\rho(g)(A)\xrightarrow{\sim}A$$

is an isomorphism for all $g\in G$, $A\in \cC$ such that

[WORK: add diagram]

commutes. The morphisms in $\cC^{G}$ are morphisms in $f$ such that

[WORK: add diagram]

commutes. [WORK: define the rest of the structure]. This endows $\cC^{G}$ with the structure of a fusion category. We call $\cC^{G}$ the \textit{equivariantization} of $G$.

[WORK: the space of possible maps $\gamma$ making $(n\cdot \bone, \gamma)$ an element of $\cC^{G}$ is equivalent to the space of $n$-dimensional representations of $G$. To make this canonical, however, one needs to choose a choice of trivialization of $\rho(g)(\bone)$ for all $g\in G$. That is, an isomorphism $\rho(g)(\bone)\cong \bone$.

I figured it out. I added the axiom that $\rho(e)$ is the identity. We thus have the distinguished map

\begin{align*}
&\rho(g)(\bone)\\
&\to\rho(g)(\bone)\otimes \rho(e)(\bone)\\
&\to\rho(g)(\bone)\otimes \rho(g)\rho(g^{1})(\bone)\\
&\to \rho(g)(\bone \otimes \rho(g^{-1})(\bone))\\
&\to \rho(g)(\rho(g^{-1})(\bone))\\
&\to \rho(e)(\bone)\\
&\to \bone
\end{align*}
...
[WORK: figure out how to pass this lemma through once there's extra structure added onto $\cC$.]
\end{prop}
...
\begin{defn}[Internal $G$-symmetry] An \textit{internal $G$-symmetry} of a fusion category $\cC$ is a fully faithful functor $\Rep(G)\xrightarrow{} \cC$.
\end{defn}
...
\begin{prop}.[WORK: define equivariantization and assert that it behaves well]
\end{prop}
\begin{proof}.[WORK: do proof]
\end{proof}
...
\begin{enumerate}
\item Given any $A\in \cC_{g}$, all other elments of $\cC_g$ an be obtained by fusion with an element of $\cC_{e}$;

\item Rank($\cC_g$)=$\#$ fixed points of $\cC_{e}$ under action of $g$
\end{enumerate}
...
\item .[WORK: show that there is a canonical bijection between structures of a left $\cC$-module on $\cM$ and $\bC$-linear monoidal functors

$$\cM\xrightarrow{}\End(\cC).$$
